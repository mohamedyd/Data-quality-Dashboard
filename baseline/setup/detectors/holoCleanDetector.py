...
def holoclean(dirty_df, dataset_name, dataset_path, detections_path):
    ...
    hc = HoloClean(**params).session

    # log to mlflow
    log_param('HoloClean', json.dumps(params))

    # Define the path to the constaints directory
    dir = dataset_path

    # try:
    #     all_constraints_file = open(os.path.join(dir, "constraints.txt"), 'w+')
    ...
    hc.load_data(dataset_name, dataset_path, df=copy_dirtydf)

    dc_path = os.path.join(dir, "constraints.txt")
    hc.load_dcs(dc_path)

    log_artifact(dc_path)  # log dcs to mlflow
