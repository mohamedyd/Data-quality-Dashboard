...
from ..features.MetaDataFeatures import MetaDataFeatures
...
def create_next_data(train, train_target, feature_matrix, target, y_pred, n, data, column_id,
                     user_error_probability=0.0, id_list=[]):
    diff = np.absolute(y_pred - 0.5)
    sorted_ids = np.argsort(diff)
    ...
    for column_id in range(dataSet.shape[1]):
        ...
        try:
            ...
            feature_name_list.extend(
                [str(dataSet.clean_pd.columns[column_id]) + "_letter_" + tuple_dict_sorted[0] + "_" for
                 tuple_dict_sorted in listed_tuples])
            ...
            if runSVD:
                ...
                from ..VisualizeSVD import visualize_svd
                visualize_svd(svd.components_, dataSet, pipeline.named_steps['vect'].vocabulary_, column_id)

                feature_name_list.extend([str(dataSet.clean_pd.columns[column_id]) + "_svd_" + str(svd_id) for svd_id in
                                          range(feature_matrix_train.shape[1] - 1)])
...
def add_metadata_features(data, train_indices, test_indices, all_features_train, all_features_test, feature_names,
                          use_meta_only=False):
...
def visualize_model(dataSet, column_id, final_gb, feature_name_list, train, target_run, res):
    try:
        ...
        path = directory + '/' + str(column_name) + '_' + str(train[column_id].shape[0]) + '_' + str(
            time.time()) + '.html'

        table_content = show_weights(final_gb[column_id], feature_names=feature_name_list_err_corr,
                                     importance_type="gain").data

        # print table_content
        from ..VisualizeSVD import replace_with_url
