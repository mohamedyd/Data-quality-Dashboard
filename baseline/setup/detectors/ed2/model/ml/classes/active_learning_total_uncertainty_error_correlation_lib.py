import pickle
from ..active_learning.library import *
import sys
from sklearn.metrics import confusion_matrix
from ..Word2VecFeatures.Word2VecFeatures import Word2VecFeatures
from ..features.ActiveCleanFeatures import ActiveCleanFeatures
from ..features.ValueCorrelationFeatures import ValueCorrelationFeatures
from ..features.BoostCleanMetaFeatures import BoostCleanMetaFeatures
from ..features.CompressedDeepFeatures import read_compressed_deep_features
...
def augment_features_with_predictions(data_x_matrix, all_matrix_train, current_predictions, column_id,
                                      train_chosen_ids):
...
def run(dataSet,
    ...
    feature_names_potential = ['distinct_values_fraction', 'labels', 'certainty', 'certainty_stddev',
                               'minimum_certainty']
    ...
    for check_this in range(checkN):
        ...
        all_matrix_train, all_matrix_test, feature_name_list = create_features(dataSet, train_indices, test_indices,
                                                                               ngrams,
        ...
        if use_lstm:
            all_matrix_train, all_matrix_test, feature_name_list = add_lstm_features(dataSet, use_lstm_only,
                                                                                     all_matrix_train,
                                                                                     feature_name_list)

        if use_word2vec:
            w2v_features = Word2VecFeatures(vector_size=w2v_size)
            all_matrix_train, all_matrix_test, feature_name_list = w2v_features.add_word2vec_features(dataSet,
                                                                                                      train_indices,
                                                                                                      test_indices,
                                                                                                      all_matrix_train,
                                                                                                      all_matrix_test,
        ...
        for round in range(label_iterations * dataSet.shape[1]):
            ...
            if rounds_per_column[column_id] == 1:
                ...
                if type(train[column_id]) == type(None):
                    ...
                    print("column " + str(column_id) + " is not applicable" + " errors: " + str(
                        np.sum(dataSet.matrix_is_error[:, column_id])))
                ...
                if type(train[column_id]) == type(None):
                    if round < dataSet.shape[1] * number_of_round_robin_rounds:
                        ...
                        column_id = go_to_next_column(dataSet, statistics, use_max_pred_change_column_selection,
                                                      use_max_error_column_selection,
                                                      use_min_certainty_column_selection, use_random_column_selection)
                    continue
                ...
                if correlationFeatures:
                    data_x_matrix, x_all = augment_features_with_predictions(data_x_matrix, all_matrix_train,
                                                                             current_predictions, column_id,
                                                                             train_chosen_ids)
                    if type(None) != type(all_matrix_test):
                        x_all_test = augment_features_with_predictions_test(all_matrix_test, current_predictions_test,
                                                                            column_id)
                ...
                eval_scores = classifier.run_cross_validation_eval(train[column_id], train_target[column_id], 7,
                                                                   column_id)
            ...
            y_pred_current_prediction, res_new, y_pred_current_prediction_test, res_new_test = classifier.train_predict_all(
                data_x_matrix,
            ...
            if column_id in res:
                ...
                print(
                    "no change 0: " + str(no_change_0) + " no change 1: " + str(no_change_1) + " sum no change: " + str(
                        no_change_0 + no_change_1))
                print("change 0 ->1: " + str(change_0_to_1) + " change 1->0: " + str(
                    change_1_to_0) + " sum change: " + str(
            ...
            save_fscore.append(
                f1_score(dataSet.matrix_is_error[train_indices, :].flatten(), all_error_status.flatten()))
            save_precision.append(
                precision_score(dataSet.matrix_is_error[train_indices, :].flatten(), all_error_status.flatten()))
            save_recall.append(
                recall_score(dataSet.matrix_is_error[train_indices, :].flatten(), all_error_status.flatten()))
            ...
            if store_results:
                np.save(
                    Config.get("logging.folder") + "/results/result" + dataSet.name + "_" + str(check_this) + "_" + str(
                        ts), all_error_status)
            ...
            for i in range(num_hist_bin):
                feature_array.append(float(len(
                    diff[np.logical_and(diff >= i * (0.5 / num_hist_bin),
                                        diff < (i + 1) * (0.5 / num_hist_bin))])) / len(
                    diff))
            ...
            for histogram_i in range(num_hist_bin):
                feature_array.append(float(len(prediction_change_y_pred[np.logical_and(
                    ...
                    prediction_change_y_pred < (histogram_i + 1) * (1.0 / num_hist_bin))])) / len(
                    prediction_change_y_pred))
            ...
            if round < dataSet.shape[1] * number_of_round_robin_rounds:
                ...
                column_id = go_to_next_column(dataSet, statistics, use_max_pred_change_column_selection,
                                              use_max_error_column_selection, use_min_certainty_column_selection,
                                              use_random_column_selection)
