...
def repair(clean_path, dirty_path, target_path, detections_path, dataset_name, repair_method):
    ...
    #     repaired_df = pd.read_csv(target_path, header="infer", encoding="utf-8", low_memory=False)
    #     return repaired_df
    ...
    if os.path.exists(detections_path):
        ...
        #print("Generate a dictionary of all dirty instances in the dataset ..")
        #detections_dictionary = get_all_errors(dirty_df, clean_df)
        return pd.DataFrame()

        if not detections_dictionary:
            raise ValueError("Clean and dirty datasets are identical (detection dictionary empty")

    if repair_method == RepairMethod.ML_IMPUTER:
        print("Running ML imputer")
        df_repaired = mlImputer(dirty_df, detections_dictionary, repair_method="separate", target_path=target_path,
                                num='decisionTree', cat='knn')
    elif repair_method == RepairMethod.STANDARD_IMPUTER:
        print("Running standard imputer")
...
if __name__ == "__main__":
    # Get the data path
    dataset_name = 'nasa'
    detector_name = DetectMethod.RAHA.__str__()
    repair_method = RepairMethod.BARAN
