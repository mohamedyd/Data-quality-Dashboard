...
from mlflow import log_param, log_params, log_artifacts
...
def mlImputer(dirty_df, detections, repair_method, target_path, **kwargs):
    ...
    if repair_method == "mix":
        ...
        params['num_method'] = num_method
        params['cat_method'] = cat_method
    ...
    if num_method == "knn":
        ...
        n_neighbors = 5
        imputer = sklearnKNNImputer(missing_values=missing_values, n_neighbors=n_neighbors)
    ...
    if num_method == "decisionTree" or num_method == "bayesianRidge" or num_method == "extraTrees":
        ...
        if num_method == "decisionTree":
            max_features = 'sqrt'
            estimator = DecisionTreeRegressor(max_features=max_features)
        ...
        missing_values = np.nan
        imputer = sklearnIterativeImputer(estimator=estimator, missing_values=missing_values)
        params['missing_values'] = missing_values
        num_repaired = imputer.fit_transform(num_df)
    ...
    if cat_method == "knn" and cat_df.shape[1] > 0:
        ...
        #imputer = MissForest()
        n_neighbors = 2
        imputer = sklearnKNNImputer(n_neighbors=n_neighbors)
        params['n_neighbors'] = n_neighbors

        #cat_repaired_enc = imputer.fit_transform(cat_X_enc.values.astype(float), cat_vars=cat_indices)

        for col in cat_X_enc.columns:
            cat_X_enc[col] = cat_X_enc[col].astype('float', errors='ignore')
        cat_repaired_enc = imputer.fit_transform(cat_X_enc)
    ...
    if mix_method == "missForest":
        ...
        # impute np.nan values
        imputer = MissForest()
        repaired_enc = imputer.fit_transform(X_enc.apply(pd.to_numeric))
    ...
    if repair_method == "mix":
        outoforder_concat = pd.concat([repaired, dirty_df[num_all_nan_cols]], axis=1)
    else:
    ...
    log_param('ML_Imputer', json.dumps(params))
